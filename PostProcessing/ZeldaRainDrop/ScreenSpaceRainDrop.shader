Shader "Hidden/XiheRendering/ScreenSpaceRainDrop" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 ComputeFragment(Varyings IN) : SV_Target
            {
                ...
                float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, IN.uv0);
                depth = Linear01Depth(depth, _ZBufferParams);

                float2 p0_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(-1 * _Thickness * depth, -1 * _Thickness * depth)) / _ScreenParams.xy;
                float2 p1_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(0, -1 * _Thickness * depth)) / _ScreenParams.xy;
                float2 p2_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(1 * _Thickness * depth, -1 * _Thickness * depth)) / _ScreenParams.xy;
                float2 p3_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(-1 * _Thickness * depth, 0)) / _ScreenParams.xy;
                float2 p5_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(1 * _Thickness * depth, 0)) / _ScreenParams.xy;
                float2 p6_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(-1 * _Thickness * depth, 1 * _Thickness * depth)) / _ScreenParams.xy;
                float2 p7_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(0, 1 * _Thickness * depth)) / _ScreenParams.xy;
                float2 p8_uv = (IN.screenPos.xy * _ScreenParams.xy + float2(1 * _Thickness * depth, 1 * _Thickness * depth)) / _ScreenParams.xy;
                ...
                //edge mask
                float edgeMask = step(_EdgeThreshold, d);
                float dir = atan2(dy, dx);
                dir = (dir + 3.14159265359) / 6.28318530718;
                float angleMask = step(_AngleThreshold, dir);
                edgeMask *= angleMask;

                //noise mask
                float2 noiseUV = IN.uv0 * _NoiseScale + _Time.xy * _DropSpeed;
                float noiseMask = step(_NoiseThreshold,SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, noiseUV).r);

                //final mask
                float mask = edgeMask * noiseMask;
                mask *= _Intensity;

                //blend
                float4 screenColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, IN.uv0);

                float4 result = float4(lerp(screenColor.rgb, _Color.rgb, mask), 1);
